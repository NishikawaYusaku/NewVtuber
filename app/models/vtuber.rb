class Vtuber < ApplicationRecord
  ...
  validates :name_x, uniqueness: true, length: { maximum: 15 }, if: -> { new_record? || changes[:name_x] }, allow_blank: true
  validates :agreement_vtuber, acceptance: { allow_nil: false, on: :create }
  ...
  def notification_update(current_user)
    ...
    Favorite.where(vtuber_id: id).pluck(:user_id).each do |user_id|
      current_user.active_notifications.create(
        ...
        created_at: time,
        updated_at: time,
        read_at: (time if current_user.id == user_id)
      )
    end
  end
